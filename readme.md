...
**👨‍💻 GitHub:** [@alfirm](https://github.com/alfirm)  
...
### Конкретный пример
```bash
dotnet nuget add source https://nuget.pkg.github.com/alfirm/index.json \
  --name github-alfirm \
  --username alfirm \
...
```bash
dotnet new install alfirm.cleanarchitecture.template
```
...
- Обновить шаблон:
  ```bash
  dotnet new install alfirm.cleanarchitecture.template --force
  ```
- Удалить шаблон:
  ```bash
  dotnet new uninstall alfirm.cleanarchitecture.template
  ```
- Удалить источник:
  ```bash
  dotnet nuget remove source github-alfirm
  ```
...
git clone https://github.com/alfirm/dotnet-clean-architecture-template.git
